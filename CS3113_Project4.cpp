...
bool allocateSegmentTable(list<MemoryBlock>& memoryList, PCB& process, int& tableAddress) {
	// Always allocate exactly 13 integers for the segment table
	const int SEGMENT_TABLE_SIZE = 13;

	// Find the first free block large enough
	for (auto it = memoryList.begin(); it != memoryList.end(); ++it) {
		if (it->processID == -1 && it->size >= SEGMENT_TABLE_SIZE) {
			// Use the start of the current free block
			tableAddress = it->startAddress;

			// Split the block if it's larger than needed
			if (it->size > SEGMENT_TABLE_SIZE) {
				MemoryBlock remainingBlock{
						-1,                               // processID (free)
						tableAddress + SEGMENT_TABLE_SIZE, // startAddress
						it->size - SEGMENT_TABLE_SIZE     // size
				};

				// Adjust current block size
				it->size = SEGMENT_TABLE_SIZE;

				// Insert remaining block after current
				memoryList.insert(next(it), remainingBlock);
			}

			// Mark block as allocated
			it->processID = process.processID;
			process.mainMemoryBase = tableAddress;
...
void setupSegmentTableAndPCB(PCB& process, vector<int>& mainMemory, int segmentTableAddress,
	...
	// Calculate logical addresses for instructions and data
	int instrLogicalBase = pcbOffset + 10; // After PCB fields
	int dataLogicalBase = instrLogicalBase + process.numOfInstruction;
	...
	mainMemory[segmentTableAddress + pcbOffset + 3] = instrLogicalBase;
	mainMemory[segmentTableAddress + pcbOffset + 4] = dataLogicalBase;
...
void tryLoadJobsToMemory(queue<PCB>& newJobQueue, queue<int>& readyQueue,
	...
	do {
		...
		if (!allocateSegmentTable(memoryList, currentProcess, segmentTableAddress)) {
			cout << "  Failed to allocate segment table" << endl;
			break;  // Can't allocate segment table
		}
		cout << "  Allocated segment table at " << segmentTableAddress << endl;
		...
		if (!allocateSegments(memoryList, currentProcess, segmentTableAddress, segments)) {
			...
			if (coalesceMemory(memoryList)) {
				cout << "  Coalesced memory, trying again" << endl;
		...
		cout << "  Allocated segments: " << endl;
		for (const auto& seg : segments) {
			cout << "    Start: " << seg.first << ", Size: " << seg.second << endl;
		}
...
void executeCPU(int segmentTableAddress, vector<int>& mainMemory, queue<int>& readyQueue,
	...
	while (programCounter < numInstructions && cyclesUsed < CPUAllocated) {
		...
		switch (opcode) {
			...
			case 3: {  // Store
				...
				// Print address translation for store instruction
			...
			case 4: {  // Load
				...
				// Print address translation for load instruction
	...
	if (programCounter < numInstructions && cyclesUsed >= CPUAllocated) {
		// Changed TimeOut to TimeOUT to match expected output
	...
	if (programCounter >= numInstructions) {
		...
		// Changed to match expected output format
